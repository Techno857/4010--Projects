...
Ghost::Ghost(int x, int y, int width, int height, ofImage spriteSheet, EntityManager* em, string color): Entity(x, y, width, height){
    ...
    float ColorSetter = ofRandom(0,1);
    if (ColorSetter <= 0.25) color = "red";
    else if (ColorSetter > 0.25 && ColorSetter <= 0.50) color = "pink";
    else if (ColorSetter > 0.50 && ColorSetter <= 0.75) color = "cyan";
    else if (ColorSetter > 0.75 && ColorSetter <= 1) color = "orange";
