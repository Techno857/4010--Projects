...
MenuState::MenuState() {
	...
	selectorButton = new Button(ofGetWidth()/2-32, ofGetHeight()/2 + 60, 64, 50, "Character Selection");
...
void MenuState::mousePressed(int x, int y, int button){
	...
	selectorButton->mousePressed(x,y);
}
...
void MenuState::reset(){
	...
	setNextState("");
	startButton->reset();
	selectorButton->reset();
}

MenuState::~MenuState(){
	delete startButton;
	delete selectorButton;
	delete anim;
}