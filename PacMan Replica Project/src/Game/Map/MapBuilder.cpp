...
Map* MapBuilder::createMap(ofImage mapImage, string ReceivedSprite){
    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBoundBlock){
                ...
                Player* PacMan = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, ReceivedSprite);
				...
				int randomizer = (ofRandom(100));
				if(randomizer <= 25){
					CherryUp* cherryUp = new CherryUp(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
					mapInCreation->addEntity(cherryUp);
				}
				else if(randomizer > 25 && randomizer <= 50){
					StrawberryUp* strawberryUp = new StrawberryUp(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
					mapInCreation->addEntity(strawberryUp);
				}
				else if(randomizer > 50 && randomizer <= 85){
					RandomUp* randomUp = new RandomUp(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
					mapInCreation->addEntity(randomUp);
				}
                else{
